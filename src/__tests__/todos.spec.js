...
describe('Todos', () => {
   it("should be able to list all user's todo", async () => {
	 ...
	 .get('/todos').set('username', userResponse.body.username);
   ...
   it('should not be able to update a non existing todo', async () => {
	 ...
	 username: 'user8'
      ...
      const response = await request(app).put('/todos/invalid-todo-id')
	 .send({
	    title: 'update title',
	    deadline: todoDate
	 }).set('username', userResponse.body.username).expect(404);
   ...
   it('should not be able to mark a non existing as done', async () => {
      const userResponse = await request(app).post('/users').send({
	 name: 'John Doe',
	 username: 'user4'
      });

      const response = await request(app).patch('/todos/ivanlid-todo-id/done')
	 .set('username', userResponse.body.username)
	 .expect(404);
   ...
   it('should be not be able to delete a non existing todo', async () => {
      const userResponse = await request(app)
	 .post('/users')
	 .send({
	    name: "John Doe",
